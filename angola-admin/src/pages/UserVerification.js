// src/pages/UserVerification.js - Version simplifiée fonctionnelle
...
const UserVerification = () => {
  ...
  const callAPI = async (endpoint, options = {}) => {
    ...
    const response = await fetch(`/api/admin/provider-verification${endpoint}`, {
      headers: {
  ...
  const fetchStatistics = async () => {
    try {
      const stats = await callAPI('/statistics/');
      setStatistics(stats);
    } catch (error) {
  ...
  const handleAction = async () => {
    try {
      ...
      switch (modalType) {
        case 'approve':
          await callAPI(`/${currentVerification.id}/approve/`, {
            method: 'POST',
        ...
        case 'reject':
          ...
          await callAPI(`/${currentVerification.id}/reject/`, {
            method: 'POST',
            body: JSON.stringify({ 
          ...
          break;
        case 'bulk_approve':
          await callAPI('/bulk-approve/', {
            method: 'POST',
            body: JSON.stringify({ 
              verification_ids: Array.from(selectedItems),
              admin_notes: adminNotes 
            })
          });
          setSelectedItems(new Set());
    ...
    <div className="px-4 sm:px-6 lg:px-8">
      ...
      <div className="sm:flex sm:items-center sm:justify-between mb-8">
        ...
        <div className="mt-4 sm:mt-0">
          <button
            onClick={() => { fetchVerifications(); fetchStatistics(); }}
              ...
              <tr>
                <td colSpan="6" className="px-6 py-12 text-center">
                  <div className="flex flex-col items-center">
                    ...
                    <p className="text-gray-500">Chargement des vérifications...</p>
                  </div>
                </td>
              </tr>
              ...
              <tr>
                <td colSpan="6" className="px-6 py-12 text-center">
                  <div className="text-gray-500">
                    <p className="text-lg font-medium mb-2">Aucune vérification trouvée</p>
                    <p className="text-sm">Essayez de modifier vos filtres ou vérifiez plus tard.</p>
                  </div>
                </td>
              </tr>
            ) : (
